...
void viterbi(HMM *hmm_ptr, char *O, FILE *fp_out, FILE *fp_aa, FILE *fp_dna,
	...
	for (t = 1; t < len_seq; t++) {
		...
		if (t == 0) {
			...
			alpha[R_STATE_1][t] = alpha[R_STATE_1][t - 1]
					- log(hmm_ptr->tr_R_R[from][to]) - log(hmm_ptr->tr[TR_RR]);
			path[R_STATE_1][t] = R_STATE_1;

			alpha[R_STATE_1][t] -= log(0.95);
		}
		...
		if (t == 0) {
			...
			alpha[R_STATE_2][t] = alpha[R_STATE_2][t - 1]
					- log(hmm_ptr->tr_R_R[from][to]);
			path[R_STATE_2][t] = R_STATE_2;

			temp_alpha = alpha[E_STATE_1][t - 1];
			if (temp_alpha < alpha[R_STATE_2][t]) {
				alpha[R_STATE_2][t] = temp_alpha;
				path[R_STATE_2][t] = E_STATE_1;
			}
			alpha[R_STATE_2][t] -= log(0.95);
	...
	alpha[E_STATE][t+2] = alpha[E_STATE][t+2] - log(p_kd);
		...
		if (alpha[S_STATE_1][t] == 0) {
			...
			if (t < len_seq - 2 && (O[t + 2] == 'A' || O[t + 2] == 'a')
				...
				alpha[S_STATE_1][t] = max_dbl;
				path[S_STATE_1][t] = R_STATE_1;
				alpha[S_STATE_1][t + 1] = max_dbl;
				alpha[S_STATE_1][t + 2] = alpha[R_STATE_1][t - 1]
...
// TO HERE
	...
	for (t = 0; t < len_seq; t++) {
		/*
		if ((vpath[t] >= M1_STATE_1 && vpath[t] <= M6_STATE_1)
			|| vpath[t] == S_STATE_1 || vpath[t] == E_STATE_1 || vpath[t] == R_STATE_1 || vpath[t] == R_STATE_2){
			start_t = 1;
			end_t = len_seq;
			fprintf(fp_out, "%d\t%d\t-\t%d\t%lf\t", start_t, end_t, 0, 0);
			fprintf(fp_out, "\n");
			break;
		}
		*/
		...
		if (codon_start == 0
			...
			if (vpath[t] < M6_STATE) {
				...
				&& (vpath[t] == E_STATE || vpath[t] == E_STATE_1 || (codon_start == -1 && t==len_seq-1))
				&& start_t != -1) {
